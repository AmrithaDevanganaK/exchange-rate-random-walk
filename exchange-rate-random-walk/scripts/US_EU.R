...
# Load Data
data <- read.csv("C:/Users/amrit/Downloads/DEXUSEU (1)_cleaned.csv",
...
# Log-transform 
train_data$LogValue <- log(train_data$DEXUSEU)
...
# KPSS Test 
kpss_result <- kpss.test(diff_series, null = "Level")  # or "Trend"
print(kpss_result)

# Compute log-returns (epsilon_t) 
epsilon_t <- diff(log(train_data$DEXUSEU))

# Define range of scaling constants K to test
k_values <- 130:150
...
# Store results and find optimal K
results_df <- data.frame(K = k_values, Proportion = proportions)
print(results_df)


# Set chosen K and discretize epsilon ---
k <- 150
epsilon_scaled <- k * epsilon_t
...
adjusted_core_mass <- adjusted_core_mass / sum(adjusted_core_mass)
...
simulate_exchange_rate_log <- function(T, N_paths = 1000, S0 = 1.0) {
  eps_matrix <- matrix(sample(epsilon_values, size = T * N_paths,
                       ...
                       nrow = T, ncol = N_paths)
  # Scale epsilon
  eps_matrix <- eps_matrix / k  # k is chosen scaling constant
...
# Define test data 
log_test_short <- log(test_data %>% slice(1:3) %>% pull(DEXUSEU))
log_test_mid   <- log(test_data %>% slice(1:90) %>% pull(DEXUSEU))
log_test_long  <- log(test_data %>% slice(1:250) %>% pull(DEXUSEU))

S0 <- test_data$DEXUSEU[1]
...
plot_short <- plot_short_log_paths(short_log_sim, "Short-Term (Log, 3 Days)", actual_log = log_test_short)
plot_mid   <- plot_log_paths(mid_log_sim, "Mid-Term (Log, 90 Days)", actual_log = log_test_mid)
plot_long  <- plot_log_paths(long_log_sim, "Long-Term (Log, 250 Days)", actual_log = log_test_long)
...
cat("RMSE Short-term (log):", rmse_short_log, "\n")
cat("RMSE Mid-term (log):", rmse_mid_log, "\n")
cat("RMSE Long-term (log):", rmse_long_log, "\n")
...
# Log-transform 
log_x <- log(train_data$DEXUSEU)
diff_logx <- diff(log_x)
...
par(mfrow=c(1,2))
acf(residuals(auto_fit), main = "ACF of ARIMA(0,1,0)")
pacf(residuals(auto_fit), main = "PACF of ARIMA(0,1,0)")


# Simulation function
simulate_arima_log <- function(fit, T, N_paths = 1000, S0 = 1.0) {
  sim_matrix <- matrix(NA, nrow = T, ncol = N_paths)
  for (i in 1:N_paths) {
    sim_log <- simulate(fit, nsim = T, future = TRUE)   
    sim_log <- sim_log + log(S0) - sim_log[1]           
...
plot_fan_chart_log <- function(sim_log_matrix, title, actual_log = NULL, short_term = FALSE) {
  days <- seq_len(nrow(sim_log_matrix))
  
  # Pick 5 random simulated paths
  idx <- sample(ncol(sim_log_matrix), 5)
  sim_subset <- sim_log_matrix[, idx, drop = FALSE]
  df_paths <- melt(sim_subset)
  colnames(df_paths) <- c("Day", "Path", "Log_Rate")
  
  plt <- ggplot() +
    geom_line(data = df_paths, aes(x = Day, y = Log_Rate, group = Path),
              color = "red", alpha = 0.6, size = 0.8)
  
  # Add actual log data if provided
  if (!is.null(actual_log)) {
    df_actual <- data.frame(Day = seq_along(actual_log), Log_Rate = actual_log)
    plt <- plt + geom_line(data = df_actual, aes(x = Day, y = Log_Rate),
                           color = "black", size = 1.0)
  }
  
  plt <- plt +
    labs(title = title, x = "Day", y = "Log Exchange Rate") +
    theme_minimal()
  
  # Only fix x-axis for short-term
  if (short_term) {
    plt <- plt + scale_x_continuous(breaks = 1:nrow(sim_log_matrix))
  }
...
log_test_short <- log(test_data$DEXUSEU[1:3])
log_test_mid   <- log(test_data$DEXUSEU[1:90])
log_test_long  <- log(test_data$DEXUSEU[1:250])
...
for (i in seq_along(horizons)) {
  ...
  sim_log <- simulate_arima_log(auto_fit, T = T_h, N_paths = 1000, S0 = S0)
  simulations[[i]] <- sim_log
  
  actual <- if(T_h == 3) log_test_short else if(T_h == 90) log_test_mid else log_test_long
  
  # Compute RMSE in log scale
  rmse_vals[i] <- rmse(actual, rowMeans(sim_log))
  
  # Rescale RMSE to percentage
...
# Log returns
log_returns <- diff(log(train_data$DEXUSEU))
...
# Simulation function 
simulate_white_noise_rw_log <- function(T, N_paths = 1000, S0 = 1.0, mu = -2.143479e-05, sigma = 0.004833462) {
...
plot_fan_chart_log <- function(log_matrix, title_str, actual_log, short_term = FALSE) {
  ...
  # Fix x-axis only for short-term
...
# Define test data 
test_short_log <- log(test_data$DEXUSEU[1:3])
test_mid_log   <- log(test_data$DEXUSEU[1:90])
test_long_log  <- log(test_data$DEXUSEU[1:250])
...
# Show plots 
print(plots_garch[[1]])  # Short-term
print(plots_garch[[2]])  # Mid-term
print(plots_garch[[3]])  # Long-term
